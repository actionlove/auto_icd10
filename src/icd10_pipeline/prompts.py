...
1. **Document, don't infer.** Include a problem only if the Doctor asserted it, confirmed it, or is actively evaluating/treating it. A patient's self-diagnosis ("I think it's my sciatica") is NOT confirmed unless the Doctor endorses it.
...
8. **Symptoms vs diagnoses**: if the Doctor gave a diagnosis, list the diagnosis; list a symptom as its own problem only if it was NOT attributed to a listed diagnosis (unexplained symptoms are separately codable).
